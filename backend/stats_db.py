...
# ---------------------------------------------------------------------------
# Мини-игра «Кто победил?» (game id: "ww") — случайный реальный матч + ответ
# ---------------------------------------------------------------------------
#
# Раунд: клиент получает драфты обеих сторон БЕЗ radiant_win и угадывает
# победителя; ответ проверяется на сервере (/api/minigames/ww/guess).
# Рекорд/лидерборд — общая minigame-инфраструктура (settings.minigame_best).

import random as _ww_random

_WW_IDS_TTL_SEC = 6 * 3600
_ww_ids: list[int] = []
_ww_ids_built_at: float = 0.0
_ww_ids_lock = threading.Lock()


def _ww_match_ids() -> list[int]:
    """Кэш списка всех match_id (TTL 6ч) для O(1)-выбора случайного матча.

    ~215k id — пара МБ на процесс. Альтернатива ORDER BY random() — seq-scan
    всей matches на КАЖДЫЙ раунд (раунд = каждые ~5-10 секунд на игрока).
    Single-flight лок — как у _MINIGAME_LB_LOCK: пересборку делает один поток.
    """
    global _ww_ids, _ww_ids_built_at
    if _ww_ids and (time.time() - _ww_ids_built_at) < _WW_IDS_TTL_SEC:
        return _ww_ids
    with _ww_ids_lock:
        if _ww_ids and (time.time() - _ww_ids_built_at) < _WW_IDS_TTL_SEC:
            return _ww_ids
        with engine.connect() as conn:
            rows = conn.execute(
                text("SELECT match_id FROM matches WHERE duration >= :d"),
                {"d": MIN_MATCH_DURATION_SECONDS},
            ).fetchall()
        ids = [r[0] for r in rows]
        if ids:
            _ww_ids = ids
            _ww_ids_built_at = time.time()
            logger.info("[stats_db] ww: match id cache rebuilt: %d ids", len(ids))
    return _ww_ids


def get_ww_random_match() -> Optional[dict]:
    """Случайный матч для раунда «Кто победил?». БЕЗ radiant_win.

    Возвращает {match_id, duration, rank_tier, patch, radiant_heroes,
    dire_heroes} или None, если в БД нет пригодных матчей. До 5 попыток —
    id-кэш живёт до 6ч, и конкретный матч мог быть удалён cleanup'ом.
    """
    ids = _ww_match_ids()
    if not ids:
        return None
    for _ in range(5):
        mid = _ww_random.choice(ids)
        with engine.connect() as conn:
            row = conn.execute(
                text(
                    "SELECT match_id, duration, avg_rank_tier, patch, "
        ...
        if row is None:
            continue
        try:
            rh = row["radiant_heroes"]
            dh = row["dire_heroes"]
            rh = rh if isinstance(rh, list) else json.loads(rh or "[]")
            dh = dh if isinstance(dh, list) else json.loads(dh or "[]")
        except (TypeError, ValueError):
            continue
        if len(rh) != 5 or len(dh) != 5:
            continue
        return {
            "match_id":       row["match_id"],
            "duration":       row["duration"],
            "rank_tier":      row["avg_rank_tier"],
            "patch":          row["patch"],
            "radiant_heroes": rh,
            "dire_heroes":    dh,
        }
...
def get_ww_match_result(match_id: int) -> Optional[dict]:
    """Ответ раунда: radiant_win + командные итоги для reveal-экрана.

    teams.{radiant,dire}.kills/net_worth — суммы по match_players; могут быть
    None, если пер-игроковых строк для матча нет (фронт тогда прячет цифры).
    """
    with engine.connect() as conn:
        row = conn.execute(
            text("SELECT radiant_win FROM matches WHERE match_id = :id"),
            {"id": match_id},
        ).fetchone()
        if row is None:
            return None
        srows = conn.execute(
            text(
                "SELECT is_radiant, SUM(kills) AS kills, SUM(net_worth) AS net_worth "
                "FROM match_players WHERE match_id = :id GROUP BY is_radiant"
            ),
            {"id": match_id},
        ).mappings().all()

    teams = {
        "radiant": {"kills": None, "net_worth": None},
        "dire":    {"kills": None, "net_worth": None},
    }
    for s in srows:
        key = "radiant" if s["is_radiant"] else "dire"
        teams[key] = {
            "kills":     int(s["kills"]) if s["kills"] is not None else None,
            "net_worth": int(s["net_worth"]) if s["net_worth"] is not None else None,
        }
    return {"radiant_win": bool(row[0]), "teams": teams}
