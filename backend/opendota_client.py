...
# ---------------------------------------------------------------------------
# Сервисный слой (без кэша — добавим позже)
# ---------------------------------------------------------------------------

async def get_hero_matchups_cached(hero_id: int) -> list[dict]:
    """Возвращает матчапы героя через OpenDota API.

    Кэширование не реализовано — добавим на следующем шаге.
    """
    return await get_hero_matchups(hero_id)
